...
var config = {
      host: 'ec2-35-161-80-252.us-west-2.compute.amazonaws.com',
      port: 22,
      username: 'ec2-user',
      privateKey: fs.readFileSync('./key/usc.pem')
    };
    var commitHash = process.env.npm_config_COMMIT ? process.env.npm_config_COMMIT : "test3";
    console.log(commitHash);
function mkdir(){
    return ssh({
              sshConfig: config
        }).exec([
        'mkdir -p /var/app/deploy/'+commitHash,
        'ln -sfn /var/app/deploy/'+commitHash+' /var/app/deploy/latest']).on('error', function(err) {
    console.log(err);
    });;
...
function deploy(){
    ...
    var upload = function(){
        return sftp({
            user: 'ec2-user',
            host: 'ec2-35-161-80-252.us-west-2.compute.amazonaws.com',
            remotePath: '/var/app/deploy/'+commitHash,
            key : './key/usc.pem'
        });
    };
...
gulp.task('deploy',['build','mkdir'], deploy);
